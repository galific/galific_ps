...
class JmCheckoutDeliveryStep extends JmCheckoutStep
{
    protected $template;
    ...
    protected $step_is_reachable = true;
    ...
    public function handleRequest(array $requestParams = array())
    {
        if (isset($requestParams['delivery_option'])) {
            ...
            $this->getCheckoutSession()->setGift(
                ...
                (isset($requestParams['gift']) &&
                    isset($requestParams['gift_message'])) ? $requestParams['gift_message'] : ''
            );
        }

        if (!empty(Tools::getValue('delivery_message'))) {
            $this->getCheckoutSession()->setMessage(Tools::getValue('delivery_message'));
        } else {
            $this->getCheckoutSession()->setMessage(false);
        }
    ...
    public function render(array $extraParams = array())
    {
        ...
        /**
         * PS-1043: Need unescape before return
         */
        $hookDisplayBeforeCarrier = htmlspecialchars_decode($hookDisplayBeforeCarrier);
        $hookDisplayAfterCarrier = htmlspecialchars_decode($hookDisplayAfterCarrier);

        return $this->renderTemplate(
            $this->getTemplate(),
            $extraParams,
            array(
                'hookDisplayBeforeCarrier' => $hookDisplayBeforeCarrier,
                'hookDisplayAfterCarrier' => $hookDisplayAfterCarrier,
                ...
                'initSoFlexibiliteEngine' => Module::isInstalled('soflexibilite')
            )
        );
    }
    ...
    protected function isModuleComplete($requestParams)
    {
        ...
        Hook::exec(
            ...
            Module::getModuleIdByName(
                $currentDeliveryOption['external_module_name']
            )
        );
        return $isComplete;
    }
