...
<h1>{l s='Your addresses' d=$module_name}</h1>
...
<form action="{$link->getModuleLink("$module_name", "address", array())|escape:'quotes':'UTF-8'}" method="post" class="std" id="add_address">
	<fieldset>
		<h3>{if isset($id_address)}{l s='Your address' d=$module_name}{else}{l s='New address' d=$module_name}{/if}</h3>
	...
	{foreach from=$ordered_adr_fields item=field_name}
		...
		*}
		...
		<p class="required text" id="adress_alias">
			...
			<input type="text" id="alias" name="alias" value="{if isset($smarty.post.alias)}{$smarty.post.alias|escape:'html':'UTF-8'}{else if isset($address->alias)}{$address->alias|escape:'html'}{elseif !$select_address}{l s='My address' d=$module_name}{/if}" />
		</p>
	</fieldset>
	<p class="submit2">
		...
		<input type="submit" name="submitAddress" id="submitAddress" value="{l s='Save' d=$module_name}" class="button" />
	</p>
</form>
