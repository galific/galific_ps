...
{block name='step_content'}
    {if $customer.is_logged && !$customer.is_guest}
        ...
        {if !isset($empty_cart_on_logout) || $empty_cart_on_logout}
            <p><small>{l s='If you sign out now, your cart will be emptied.' d=$module_name}</small></p>
        {/if}
    ...
    {else}
        ...
        <ul class="nav nav-inline my-2" role="tablist">
            <li class="nav-item">
                ...
                >
                    {if $guest_allowed}
                        {l s='Order as a guest' d=$module_name}
                    {else}
                        {l s='Create an account' d=$module_name}
                    {/if}
                </a>
            </li>
