	...
	$(document).on('click', '.prdctfltr_c_add.pf_advanced', function() {
		...
		$.post(prdctfltr.ajax, curr_data, function(response) {
			if (response) {
				...
				reload_filter_titles();
				pf_add_title_disable('full',$('.pf_element.adv[data-id="'+response[0]+'"]'));
...
/*		if ( confirm(prdctfltr.localization.remove_override) === false ) {
					...
					if ( !--curr_form_el_count ) {
						...
						$.each(inputs, function(i, obj) {
							...
							if ( !--input_count ) {
								...
								if ( curr_preset.wc_settings_prdctfltr_active_filters !== undefined ) {
									...
									$.each(curr_preset.wc_settings_prdctfltr_active_filters, function(index, pf_filter) {
										...
										if ( pf_filter == 'meta' ) {
											...
											var curr_set = __get_element(curr_form.find('p span[class=wcpfs_'+pf_filter+']'));
										...
										if ( !--curr_count ) {
											$('.prdctfltr_customizer_fields .prdctfltr_c_add_filter:not(.pf_active)').each( function() {
												...
												var curr_set =  __get_element(curr_form.find('p span[class=wcpfs_'+curr_filter+']'));
								...
								pf_add_title_disable('compact',false);
	...
	$(document).on('click', '.prdctfltr_c_add.pf_range', function() {
		...
		$.post(prdctfltr.ajax, curr_data, function(response) {
			if (response) {
				...
				get_range_customizations();
				reload_filter_titles();
				pf_add_title_disable('full',$('.pf_element.rng[data-id="'+response[0]+'"]'));
	...
	$(document).ready(function() {
		...
		pf_add_title_disable('full',false);
	});

	function pf_add_title_disable(mode,where) {
		var findHere = ( where === false ? $('input[id^="pfa_title"],input[id^="pfr_title"],input[id^="pfm_title"], input[name$="_title"]:not(#wc_settings_prdctfltr_title)') : $(where).find('input[id^="pfa_title"],input[id^="pfr_title"],input[id^="pfm_title"], input[name$="_title"]:not(#wc_settings_prdctfltr_title)') );
		findHere.each( function() {
			$(this).prop('disabled', $(this).val()=='false'?true:false);
			$(this).next().prop('checked', $(this).val()=='false'?true:false);
			if ( mode=='full') {
	...
	$(document).on('click', '.prdctfltr_c_add.pf_meta', function() {
		...
		$.post(prdctfltr.ajax, curr_data, function(response) {
			if (response) {
				...
				pf_add_title_disable('full',$('.pf_element.mta[data-id="'+response[0]+'"]'));
